...
// --- Frontend HTML (With State Persistence) ---
const getHtml = (groups, selectedGid, settings) => {
    // Agar Dashboard view hai
    if (!selectedGid) {
    ...
    // Settings View (FIXED TOGGLE STATE)
    // Hum settings.antiLink ko check kar rahe hain, agar true hai toh "checked" attribute add hoga
    const isChecked = settings && settings.antiLink === true ? 'checked' : '';
    ...
    <body>
        ...
        <div class="card">
            ...
            <div class="row">
                ...
                <label class="switch">
                    <input type="checkbox" id="links" ${isChecked}>
...
bot.on('message', async (ctx, next) => {
    ...
    // Anti-link Logic check
    if (config && config.antiLink === true) {
        if (ctx.message.entities?.some(e => e.type === 'url' || e.type === 'text_link')) {
            await ctx.deleteMessage().catch(() => {});
            return; // Link delete hone ke baad aage mat badho
        }
    }
    return next();
});
...
module.exports = async (req, res) => {
    ...
    if (req.method === 'GET') {
        ...
        // Database se current settings fetch karna zaruri hai
