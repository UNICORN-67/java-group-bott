...
async function getSmartAIReply(userMessage, userName, historyContext) {
    try {
        const response = await axios.post('https://api.openai.com/v1/chat/completions', {
            model: "gpt-3.5-turbo",
            messages: [
                { role: "system", content: `You are Yuri, a witty, real girl. Talk in natural Hinglish (Roman English). Context: ${historyContext}. Short, spicy, and human-like. Don't be a bot, be a friend.` },
...
bot.command(['ping', 'speed', 'leaderboard'], async (ctx) => {
    ...
    if (cmd === 'leaderboard') {
        ...
        let res = `🏆 <b>ɢʀᴏᴜᴘ ʟᴇᴀᴅᴇʀʙᴏᴀʀᴅ</b>\n\n`;
        top.forEach((u, i) => res += `${i<3?['🥇','🥈','🥉'][i]:'👤'} <b>${u.name}</b>\n`);
        return fullClean(ctx, (await ctx.reply(res, { parse_mode: 'HTML' })).message_id, 30000);
    }

    const start = Date.now();
    const m = await ctx.reply('🚀 Testing...');
    await ctx.telegram.editMessageText(ctx.chat.id, m.message_id, null, `⚡ Ping: ${Date.now() - start}ms | Server: Vercel`);
    fullClean(ctx, m.message_id);
});

// --- 5. MANAGEMENT COMMANDS (BAN, UNBAN, MUTE, UNMUTE, PIN, UNPIN, PURGE) ---
bot.command(['ban', 'unban', 'mute', 'unmute', 'pin', 'unpin', 'purge', 'tagall', 'cancel', 'info'], async (ctx) => {
    ...
    if (cmd === 'tagall') {
        ...
        await ctx.reply("🚀 Tagging Shuru! /cancel to stop.");
        for (let i = 0; i < members.length; i += 5) {
            if (!taggingProcess[gid]) break;
            let mentions = members.slice(i, i + 5).map(u => `<a href="tg://user?id=${u.uid}">🔹</a>`).join(' ');
            await ctx.reply(mentions, { parse_mode: 'HTML' }).catch(() => {});
            await new Promise(r => setTimeout(r, 3000));
        }
        return delete taggingProcess[gid];
    }
    if (cmd === 'cancel') { taggingProcess[gid] = false; return ctx.reply("🛑 Cancelled!"); }

    // Reply-based Admin actions
    ...
    try {
        ...
        if (cmd === 'purge') {
            for (let i = target.message_id; i <= ctx.message.message_id; i++) await ctx.telegram.deleteMessage(ctx.chat.id, i).catch(() => {});
        }
        fullClean(ctx, (await ctx.reply(`✅ Action: ${cmd.toUpperCase()}`)).message_id, 5000);
